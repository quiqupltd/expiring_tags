...
describe 'Managing tags via names' do
  ...
  it 'adds tags via their names' do
    ...
    expect(venue.tags.map(&:name)).to eq ['no_takeaway']
  end

  it 'accepts a completely new set of tags' do
    venue.tag_names = ['no_takeaway', 'not_serving']

    expect(venue.tags.map(&:name)).to eq ['no_takeaway', 'not_serving']
  end
  ...
  it 'enumerates through tag names' do
    ...
    expect(names).to eq ['no_takeaway', 'not_serving']
  end
  ...
  it 'appends tag names' do
    ...
    expect(venue.tags.map(&:name)).to eq ['no_takeaway', 'not_serving', 'other']
  end
